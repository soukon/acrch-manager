...
import co.acrch.system.service.MenuService;
...
public class ShiroRealm extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
        ...
        if (user == null) {
            throw new UnknownAccountException("該当ユーザが存在しません、ご確認ください！");
        }
        if (!password.equals(user.getPassword())) {
            throw new IncorrectCredentialsException("アカウントまたはパスワードが間違いました！");
        }
        if (User.STATUS_LOCK.equals(user.getStatus())) {
            throw new LockedAccountException("該当アカウントがロックされました、管理者に連絡ください！");
        }
